...
from src.graph.sql_reviewer import sql_reviewer_node, needs_review
...
def route_after_review(state: AgentState) -> str:
    ...
    if verdict == "rejected" and retry < MAX_RETRY_COUNT:
        return "text2sql"
    if verdict == "rejected":
        return "end"
    return "execute"
...
def build_workflow() -> StateGraph:
    ...
    workflow.add_node("sql_reviewer", sql_reviewer_node)
    ...
    workflow.add_conditional_edges(
        "text2sql",
        route_review,
        {"review": "sql_reviewer", "skip_review": "execute_sql"},
    )

    workflow.add_conditional_edges(
        "sql_reviewer",
        route_after_review,
        {"execute": "execute_sql", "text2sql": "text2sql", "end": END},
    )

    workflow.add_conditional_edges(
        "execute_sql",
        route_execute,
        {"generate_chart": "generate_chart", "text2sql": "text2sql", "end": END},
    )
